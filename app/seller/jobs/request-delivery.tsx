...
export function Delivery({ jobId }: Props) {
  ...
  const handleUpload = async (filesToUpload: File[]) => {
    ...
    try {
      const uploadPromises = filesToUpload.map(async (file) => {
        ...
        const storageRef = ref(storage, `deliveries/${jobId}/${filename}`);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setOpen}>
      <DialogTrigger asChild className="bg-transparent">
        <div className="flex-1 flex items-center gap-2 cursor-pointer p-3 border rounded-2xl hover:bg-gray-50 transition-colors">
          <LuPlus className="h-4 w-4" />
          <span className="text-sm font-medium">Deliver Final Project</span>
        </div>
      </DialogTrigger>
                ...
                <div className="px-3 py-2 bg-gray-50 border rounded-lg flex gap-2 overflow-x-auto">
                  ...
                  {isUploading && files.length === 0 && (
                    <div className="flex items-center justify-center w-full py-2">
                      <FiLoader className="animate-spin text-gray-500 mr-2 h-4 w-4" />
